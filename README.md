...
- **A newly elected leader** re-queries the source and re-publishes, so a refresh the previous leader made just before dying is never lost. This runs whether or not a `SnapshotStore` is configured.
