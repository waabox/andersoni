...
- **Snapshot Patches**: `Catalog.replace(indexName, key, newItem)` swaps a single item via copy-on-write — only buckets whose keys change are rebuilt, the rest are shared by reference. `Andersoni.replaceAndSync(...)` runs leader-only, saves the snapshot to S3, and publishes a `PatchEvent` (sealed `SyncMessage` hierarchy alongside `RefreshEvent`). Followers apply version-keyed dispatch (discard stale / surgical apply / full-reload fallback). Lookup must resolve to exactly one item — `AmbiguousReplaceException` otherwise. Kafka and HTTP transports carry patch events; the DB polling transport remains refresh-only.
